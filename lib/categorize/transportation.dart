...
class TransportationWidget extends StatefulWidget {
  const TransportationWidget({Key? key}) : super(key: key);

  @override
  _TransportationWidgetState createState() => _TransportationWidgetState();
}

class _TransportationWidgetState extends State<TransportationWidget> {
  TextEditingController? budgetController;
  bool isEditable = true; // Allows editing only if no budget exists.
  String displayBudget = '0'; // Displayed budget value.
  final String categoryName = 'TRANSPORTATION';
  String? token; // Loaded from SharedPreferences.
  // Backend base URL for local development.
  final String backendBaseUrl = 'http://10.0.2.2:8000/api/categorize';

  // Variable to store total expense for the category.
  String _expense = "0";

  // Flag to track if a budget record exists.
  ...
  Future<void> fetchBudget() async {
    ...
    final url = Uri.parse(
        '$backendBaseUrl/budget/?token=$token&category_name=$categoryName');
    try {
  ...
  Future<void> fetchExpense() async {
    ...
    // Construct the GET URL for fetching expenses for the current month.
    final url = Uri.parse(
        '$backendBaseUrl/expense/?token=$token&category_name=$categoryName');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
            ...
            child: Row(
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    // Allow user to re-enable editing.
                    setState(() {
                      isEditable = true;
                    });
                  },
